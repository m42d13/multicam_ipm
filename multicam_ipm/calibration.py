...
def load_cameras(
    ...
    for index, key in enumerate(keys):
        ...
        camera_from_ego = (
            ...
            else _camera_from_embedded_pose(config, key, camera_from_cam0 @ cam0_from_ego)
        )
...
def _camera_from_embedded_pose(config: dict, camera_key: str, fallback: np.ndarray) -> np.ndarray:
    """Use an optional ``T_ego_camera`` matrix embedded beside Kalibr data."""
    if 'T_ego_camera' not in config:
        return fallback
    ego_from_camera = np.asarray(config['T_ego_camera'], dtype=np.float64)
    if ego_from_camera.shape != (4, 4):
        raise RuntimeError(f'{camera_key}.T_ego_camera must be a 4x4 transform')
    if not np.allclose(ego_from_camera[3], [0.0, 0.0, 0.0, 1.0]):
        raise RuntimeError(f'{camera_key}.T_ego_camera has an invalid final row')
    return np.linalg.inv(ego_from_camera)
